import { PrismaClient, WorkflowPath, Status } from '@prisma/client';
import { triggerCitizenAdvocate } from '../agents/citizen-advocate.agent';
...
export const routeReport = async (reportId: string): Promise<void> => {
    try {
        const report = await prisma.report.findUnique({ where: { id: reportId } });
        if (!report) return;

        let path: WorkflowPath = 'AGENT'; // Default fallback
        ...
        if (path === 'AGENT') {
            triggerCitizenAdvocate(updatedReport.id).catch(err => {
