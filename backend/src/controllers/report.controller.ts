...
import { analyzeInfrastructureImage } from '../services/ai.service';
import { routeReport } from '../services/routing.service';
...
export const submitReport = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        let aiResult;
        try {
            aiResult = await analyzeInfrastructureImage(imageUrls, description, category);
        } catch (error) {
        ...
        routeReport(report.id).catch(err => console.error("Router error:", err));
...
export const getPublicReports = async (req: Request, res: Response): Promise<void> => {
    try {
